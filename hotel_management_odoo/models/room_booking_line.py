...
class RoomBookingLine(models.Model):
    ...
    booking_id = fields.Many2one("room.booking",
                            string="Réservation",
                            help="Indique la réservation associée à la chambre",
                            ondelete="cascade"
                        )

    checkin_date = fields.Datetime(string="Date d'enregistrement",
                                    help="Vous pouvez choisir la date, sinon elle sera définie sur la date actuelle",
                                    required=True
                                )
    checkout_date = fields.Datetime(string="Date de départ",
                                    help="Vous pouvez choisir la date, sinon elle sera définie sur la date actuelle",
                                    required=True
                                )
    room_id = fields.Many2one('hotel.room',
                            string="Chambre",
                            help="Indique la chambre associée à la réservation",
                            required=True
                        )

    uom_qty = fields.Float(string="Durée (en jours)",
    ...
    price_subtotal = fields.Float(string="Sous-total",
                                  compute='_compute_price_subtotal',
                                  help="Total Price excluding Tax",
                                  store=True)
    price_tax = fields.Float(string="Total Taxes",
                             compute='_compute_price_subtotal',
                             help="Tax Amount",
                             store=True)
    ...
    state = fields.Selection(related='booking_id.state',
                             string="Status de la commande",
                             help=" Status of the Order",
                             copy=False)
    booking_line_visible = fields.Boolean(default=False,
                                          string="Ligne de réservation visible",
