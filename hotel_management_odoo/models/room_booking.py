...
class RoomBooking(models.Model):
    ...
    name = fields.Char(string="Numéro de reservation", readonly=True, index=True,
                       default="New", help="Nom de la réservation",)
    ...
    partner_id = fields.Many2one('res.partner', string="Client",
                                 ...
                                 domain="[('type', '!=', 'private'),"
                                        " ('company_id', 'in', "
                                        "(False, company_id))]")
    date_order = fields.Datetime(string="Date de reservation",
                                 required=True, copy=False,
                                 help="Creation date of draft/sent orders,"
                                      " Confirmation date of confirmed orders",
                                 default=fields.Datetime.now)
    is_checkin = fields.Boolean(default=False, string="Enregistrement effectué",
                                help="Passe à True si la chambre est occupée")
    ...
    checkin_date = fields.Datetime(
                                    ...
                                    help="Date et heure de l'enregistrement",
                                    default=fields.Datetime.now
                                )
    checkout_date = fields.Datetime(string="Départ",
                                    help="Date et heure du départ",
                                    default=fields.Datetime.now() + timedelta(
                                        hours=23, minutes=59, seconds=59))
    hotel_policy = fields.Selection([
                                        ("prepaid", "À la réservation"),
                                        ("manual", "À l'enregistrement"),
                                        ("picking", "Au départ"),
                                        ],
                                        default="manual",
                                        string="Politique de l'hôtel",
                                        help="Politique de paiement de l'hôtel : le client doit payer soit à la réservation, à l'enregistrement, ou au départ.",
                                        tracking=True
                                    )

    duration = fields.Integer(
                                string="Durée en jours",
                                help="Nombre de jours calculé automatiquement à partir des dates d'arrivée et de départ."
                            )

    invoice_button_visible = fields.Boolean(
                                        string="Affichage du bouton Facture",
                                        help="Le bouton de facture sera visible si cette option est activée."
                                    )
    ...
    currency_id = fields.Many2one(
        string="Devise", help="This is the Currency used",
    ...
    def get_details(self):
        ...
        tz_name = self.env.user.tz or False
        ...
        available_room = self.env['hotel.room'].search([('status', '=', 'available')])
        reservation = self.env['room.booking'].search_count([('state', '=', 'reserved')])

        # Check-outs today
        check_out = sum(
            1 for rec in self.env['room.booking'].search([])
            for room in rec.room_line_ids
            if room.checkout_date and room.checkout_date.date() == context_today.date()
        )

        # Staff count: users in the default internal user group
        ...
        booked_vehicles = self.env['fleet.booking.line'].search_count([('state', '=', 'check_in')])
        available_vehicle = total_vehicle - booked_vehicles

        # Events
        events = self.env['event.event'].search([])
        total_event = len(events)
        pending_events = sum(1 for event in events if event.date_end and event.date_end >= fields.Datetime.now())
        today_events = sum(1 for event in events if event.date_end and event.date_end.date() == fields.Date.today())
        ...
        for rec in self.env['account.move'].search([('payment_state', 'in', ['not_paid', 'partial'])]):
            if rec.ref and 'BOOKING' in rec.ref:
                if rec.payment_state == 'not_paid':
